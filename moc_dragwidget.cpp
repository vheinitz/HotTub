/****************************************************************************
** Meta object code from reading C++ file 'dragwidget.h'
**
** Created: Sat Jan 22 18:43:58 2011
**      by: The Qt Meta Object Compiler version 62 (Qt 4.6.3)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "dragwidget.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'dragwidget.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 62
#error "This file was generated using the moc from 4.6.3. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_DragWidget[] = {

 // content:
       4,       // revision
       0,       // classname
       0,    0, // classinfo
       6,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
      12,   11,   11,   11, 0x08,
      27,   11,   11,   11, 0x08,
      38,   11,   11,   11, 0x08,
      59,   11,   11,   11, 0x08,
      90,   11,   11,   11, 0x08,
     108,   11,   11,   11, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_DragWidget[] = {
    "DragWidget\0\0beginEditing()\0addField()\0"
    "changeButtonPushed()\0"
    "widgetRemoved(TemplateWidget*)\0"
    "hideAttachments()\0showAttachments()\0"
};

const QMetaObject DragWidget::staticMetaObject = {
    { &QMainWindow::staticMetaObject, qt_meta_stringdata_DragWidget,
      qt_meta_data_DragWidget, 0 }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &DragWidget::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *DragWidget::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *DragWidget::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_DragWidget))
        return static_cast<void*>(const_cast< DragWidget*>(this));
    return QMainWindow::qt_metacast(_clname);
}

int DragWidget::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QMainWindow::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: beginEditing(); break;
        case 1: addField(); break;
        case 2: changeButtonPushed(); break;
        case 3: widgetRemoved((*reinterpret_cast< TemplateWidget*(*)>(_a[1]))); break;
        case 4: hideAttachments(); break;
        case 5: showAttachments(); break;
        default: ;
        }
        _id -= 6;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
